...
**Before writing ANY file, resolve the correct output base path:**

- **Comprehensive mode** (or no units): write to `{SPECS_DIR}/{feature}/`
- **Incremental mode** (generating tasks for a specific unit): write to `{SPECS_DIR}/{feature}/units/{unit}/`

**Concrete example** — feature = `my-feature`, unit = `auth`:

| Mode | Tasks file path | Decisions file path |
|---|---|---|
| Comprehensive | `.aidlc/specs/my-feature/tasks.md` | `.aidlc/workflow/my-feature/decisions-tasks.md` |
| Incremental (unit: auth) | `.aidlc/specs/my-feature/units/auth/tasks.md` | `.aidlc/workflow/my-feature/units/auth/decisions-tasks.md` |
| Incremental (unit: payments) | `.aidlc/specs/my-feature/units/payments/tasks.md` | `.aidlc/workflow/my-feature/units/payments/decisions-tasks.md` |

**NEVER write unit-scoped artifacts to the shared `{SPECS_DIR}/{feature}/` directory.** The shared directory is reserved for project-wide artifacts (context.md, requirements.md, units.md, foundation.md). Each unit's tasks go in its own `units/{unit}/` subdirectory.
...
IF incremental mode (unit is set):
  ...
  DECISIONS_OUT = {WORKFLOW_DIR}/{feature}/units/{unit}/decisions-tasks.md
  DESIGN_IN = {SPECS_DIR}/{feature}/units/{unit}/design/*
ELSE (comprehensive or no units):
  TASKS_OUT = {SPECS_DIR}/{feature}/tasks.md
  DECISIONS_OUT = {WORKFLOW_DIR}/{feature}/decisions-tasks.md
